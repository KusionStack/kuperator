...
func TestRuleSet(t *testing.T) {
	...
	g.Expect(c.List(ctx, ruleSetList, &client.ListOptions{FieldSelector: fields.OneTermEqualSelector(inject.FieldIndexRuleSet, "pod-test-2")})).NotTo(gomega.HaveOccurred())
	g.Expect(len(ruleSetList.Items)).Should(gomega.BeEquivalentTo(1))
	g.Expect(c.List(ctx, ruleSetList, &client.ListOptions{FieldSelector: fields.OneTermEqualSelector(inject.FieldIndexRuleSet, "pod-test-3")})).NotTo(gomega.HaveOccurred())
	g.Expect(len(ruleSetList.Items)).Should(gomega.BeEquivalentTo(1))
	g.Expect(c.List(ctx, ruleSetList, &client.ListOptions{FieldSelector: fields.OneTermEqualSelector(inject.FieldIndexRuleSet, "pod-test-4")})).NotTo(gomega.HaveOccurred())
	g.Expect(len(ruleSetList.Items)).Should(gomega.BeEquivalentTo(1))
