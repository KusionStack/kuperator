...
import (
	...
	utilshttp "kusionstack.io/kafed/pkg/utils/http"
)
...
func (r *WebhookRuler) Filter(ruleSet *appsv1alpha1.RuleSet, targets map[string]*corev1.Pod, subjects sets.String) *FilterResult {
	return GetWebhook(ruleSet, r.Name)[0].Do(targets, subjects)
}
...
func GetWebhook(rs *appsv1alpha1.RuleSet, names ...string) (webs []*Webhook) {
	ruleNames := sets.NewString(names...)
	for i, rule := range rs.Spec.Rules {
		var web *appsv1alpha1.RuleSetRuleWebhook
		if rule.Webhook == nil {
			continue
		}
