...
.PHONY: docker-build-antgroup
docker-build-antgroup: ## Build docker image with the manager.
	docker build -f ./Dockerfile_antgroup . -t ${IMG}
...
e2e-local-deploy: docker-build-antgroup deploy-kind kind-kube-config sync-kind-image deploy
